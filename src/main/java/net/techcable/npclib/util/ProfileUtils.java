...
/** Utilities to lookup player names and uuids from mojang This caches results so you won't have issues with the rate limit
 * <p>
 * <b>DONT Rely on Bukkit.getOfflinePlayer()</b> It doesn't cache and is a workaround solution evilmidgets fetchers are a fair solution, but they
 * don't cache so you can run into ratelimits
 * 
 * @author Techcable */
public class ProfileUtils
{
	...
	/** Lookup a profile with the given name The reuturned player profile doesn't include properties If properties are neaded, proceed to use a uuid
	 * lookup
	 * 
	 * @param name
	 *            look for a profile with this name
	...
	/** Lookup a profile with the given uuid The reuturned player profile includes properties
	 * 
	 * @param id
	 *            look for a profile with this uuid
	...
	/** Represnents a player Contains their uuid and username This may or may not have properties
	 * 
	 * @author Techcable */
	public static class PlayerProfile
	{
		...
		/** Get a json array with this players properties Returns null if this players propeties haven't been retreived
		 * 
	...
	private static UUID toUUID(String raw)
	{
		...
		if (raw.length() == 32)
		{
			dashed = raw.substring(0, 8) + "-" + raw.substring(8, 12) + "-" + raw.substring(12, 16) + "-" + raw.substring(16, 20) + "-"
					+ raw.substring(20, 32);
		} else
		{
			dashed = raw;
