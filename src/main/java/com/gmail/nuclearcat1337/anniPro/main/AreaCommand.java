			...
			if (player.hasPermission("A.Area"))
			{
				if (args.length > 0)
				{
						...
						if (corner1 != null && corner2 != null && corner1.size() == 1 && corner2.size() == 1)
						{
							...
							if (one != null && two != null)
							{
								...
								if (areas != null)
								{
									...
									areas.addArea(a);
									sender.sendMessage(
											ChatColor.LIGHT_PURPLE + "Area " + ChatColor.GOLD + a.getName() + ChatColor.LIGHT_PURPLE + " added");
					...
					} else if (args[0].equalsIgnoreCase("delete") && args.length > 1)
					{
						...
						sender.sendMessage(ChatColor.LIGHT_PURPLE + "/Area " + ChatColor.GOLD + "[create,delete] [name] [allow-PvP?]");
				} else
					sender.sendMessage(ChatColor.LIGHT_PURPLE + "/Area " + ChatColor.GOLD + "[create,delete] [name] [allow-PvP?]");
			} else
