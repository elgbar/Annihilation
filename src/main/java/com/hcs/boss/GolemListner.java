...
//		System.out.println("victim " + victim.toString());
		...
		if (victimType.equals(EntityType.IRON_GOLEM))  //victim is an iron golem
		{
			if (attackerType.equals(EntityType.ARROW) && !event.isCancelled()) 
			{
				...
				event.setDamage(event.getFinalDamage() / 4); //cut the final dammage in four
...
//		else if ((Player) victim != null && attacker.getType().equals(EntityType.IRON_GOLEM)){ //victim is a player & attacker is an iron golem
